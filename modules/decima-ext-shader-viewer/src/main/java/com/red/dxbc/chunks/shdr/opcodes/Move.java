...
public final class Move extends Opcode {

    public final OperandToken0 output;
    public final OperandToken0 input;


    public Move(int opcodeToken, @NotNull ByteBuffer buffer) {
    ...
    public String toString() {
        return "mov %s, %s".formatted(output, input);
    }

    @Override
    public List<Element> toExpressions(@NotNull DXBC shader) {
        Element input = this.input.toExpression(shader, output.componentSelection);
        if (isSaturated())
            input = new Function("saturate", input);
        return List.of(new Assignment(output.toExpression(shader), input));
    }
}
