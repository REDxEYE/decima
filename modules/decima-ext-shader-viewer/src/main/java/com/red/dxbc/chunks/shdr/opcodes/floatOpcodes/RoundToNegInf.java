...
public final class RoundToNegInf extends Opcode {

    public final OperandToken0 output;
    public final OperandToken0 input;


    public RoundToNegInf(int opcodeToken, @NotNull ByteBuffer buffer) {
    ...
    public String toString() {
        return "round_ni %s, %s".formatted(output, input);
    }

    @Override
    public List<Element> toExpressions(@NotNull DXBC shader) {
        Function function = new Function("roundToNegInf", input.toExpression(shader, output.componentSelection));
        if (isSaturated()) {
