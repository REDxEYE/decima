...
public class Gather4WithOffset extends Opcode {
    ...
    public Gather4WithOffset(int opcodeToken, @NotNull ByteBuffer buffer) {
        super(opcodeToken);
    ...
    public String toString() {
        return "gather4_po %s, %s, %s, %s, %s".formatted(output, inputAddress, inputOffset, inputResource, inputSampler);
    }
    ...
    public List<Element> toExpressions(@NotNull DXBC shader) {
        ...
        return List.of(
            new Assignment(output.toExpression(shader),
                new Expression("%s.GatherWithOffset(%s, %s, %s)", arguments
                )
            )
        );
    }
