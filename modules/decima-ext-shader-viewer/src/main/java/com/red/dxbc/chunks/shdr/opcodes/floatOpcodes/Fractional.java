...
public final class Fractional extends Opcode {

    public final OperandToken0 output;
    public final OperandToken0 input;


    public Fractional(int opcodeToken, @NotNull ByteBuffer buffer) {
    ...
    public String toString() {
        return "frc %s, %s".formatted(output, input);
    }

    @Override
    public List<Element> toExpressions(@NotNull DXBC shader) {
        Function function = new Function("fract", input.toExpression(shader, output.componentSelection));
        if (isSaturated()) {
