...
public final class FloatToUInt extends Opcode {
    public final OperandToken0 output;
    public final OperandToken0 input;


    public FloatToUInt(int opcodeToken, @NotNull ByteBuffer buffer) {
        super(opcodeToken);
    ...
    public String toString() {
        return "ftou %s, %s".formatted(output, input);
    }


    @Override
    public List<Element> toExpressions(@NotNull DXBC shader) {
        Function function = new Function("uint", input.toExpression(shader, output.componentSelection));
