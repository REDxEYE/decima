...
public final class UIntToFloat extends Opcode {
    public final OperandToken0 output;
    public final OperandToken0 input;


    public UIntToFloat(int opcodeToken, @NotNull ByteBuffer buffer) {
        super(opcodeToken);
        output = new OperandToken0(buffer, true);
    ...
    public String toString() {
        return "utof %s, %s".formatted(output, input);
    }

    @Override
    public List<Element> toExpressions(@NotNull DXBC shader) {
        Function function = new Function("float", input.toExpression(shader, output.componentSelection));
        if (isSaturated()) {
