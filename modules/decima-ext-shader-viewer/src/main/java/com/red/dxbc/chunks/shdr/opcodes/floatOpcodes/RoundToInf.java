...
public final class RoundToInf extends Opcode {

    public final OperandToken0 output;
    public final OperandToken0 input;


    public RoundToInf(int opcodeToken, @NotNull ByteBuffer buffer) {
    ...
    public String toString() {
        return "round_pi %s, %s".formatted(output, input);
    }

    @Override
    public List<Element> toExpressions(@NotNull DXBC shader) {
        Function function = new Function("roundToInf", input.toExpression(shader, output.componentSelection));
        if (isSaturated()) {
