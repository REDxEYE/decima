...
public final class SquareRoot extends Opcode {

    public final OperandToken0 output;
    public final OperandToken0 input;


    public SquareRoot(int opcodeToken, @NotNull ByteBuffer buffer) {
    ...
    public String toString() {
        return "sqrt %s, %s".formatted(output, input);
    }

    @Override
    public List<Element> toExpressions(@NotNull DXBC shader) {
        Function function = new Function("sqrt", input.toExpression(shader, output.componentSelection)
        );
        if (isSaturated()) {
