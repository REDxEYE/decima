...
public final class Fetch extends Opcode {
    ...
    public Fetch(int opcodeToken, @NotNull ByteBuffer buffer) {
        super(opcodeToken);
        if (isExtended()) {
            final InstructionTokenExtendedType extendedType = IOUtils.getEnum(InstructionTokenExtendedType.class, buffer.get(6));
    ...
    public String toString() {
        return "ld %s, %s, %s".formatted(output, inputAddress, inputResource);
    }
        ...
        if (sampleOffsets != null) {
            ...
            return List.of(
                new Assignment(output.toExpression(shader),
                    new Expression("%s.Fetch(%s" + aoffimmi + ")", arguments)
                )
            );
        }
        return List.of(
            new Assignment(output.toExpression(shader),
                new Expression("%s.Fetch(%s)", arguments
                )
            )
        );
    }
