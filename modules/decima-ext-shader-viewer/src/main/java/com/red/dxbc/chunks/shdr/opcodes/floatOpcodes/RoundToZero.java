...
public final class RoundToZero extends Opcode {

    public final OperandToken0 output;
    public final OperandToken0 input;


    public RoundToZero(int opcodeToken, @NotNull ByteBuffer buffer) {
    ...
    public String toString() {
        return "round_z %s, %s".formatted(output, input);
    }

    @Override
    public List<Element> toExpressions(@NotNull DXBC shader) {
        Function function = new Function("roundToZero", input.toExpression(shader, output.componentSelection));
        if (isSaturated()) {
