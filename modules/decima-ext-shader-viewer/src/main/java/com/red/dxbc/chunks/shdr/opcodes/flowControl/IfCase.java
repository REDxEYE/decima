...
public final class IfCase extends Opcode {
    public final OperandToken0 input;

    public IfCase(int opcodeToken, @NotNull ByteBuffer buffer) {
        super(opcodeToken);
    ...
    public String toString() {
        return switch (getTestBoolean()) {
            case TEST_ZERO -> "if_z %s".formatted(input);
            case TEST_NONZERO -> "if_nz %s".formatted(input);
        };
    }
