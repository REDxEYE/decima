...
public class Gather4Comparison extends Opcode {
    ...
    public Gather4Comparison(int opcodeToken, @NotNull ByteBuffer buffer) {
        super(opcodeToken);
    ...
    public String toString() {
        return "gather4_c %s, %s, %s, %s, %s".formatted(output, inputAddress, inputResource, inputSampler, referenceValue);
    }
    ...
    public List<Element> toExpressions(@NotNull DXBC shader) {
        ...
        return List.of(
            new Assignment(output.toExpression(shader),
                new Expression("%s.GatherComparison(%s, %s, %s)", arguments
                )
            )
        );
    }
