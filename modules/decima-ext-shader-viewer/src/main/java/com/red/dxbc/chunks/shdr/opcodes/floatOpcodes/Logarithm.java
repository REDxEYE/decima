...
public final class Logarithm extends Opcode {

    public final OperandToken0 output;
    public final OperandToken0 input;


    public Logarithm(int opcodeToken, @NotNull ByteBuffer buffer) {
    ...
    public String toString() {
        return "log %s, %s".formatted(output, input);
    }

    @Override
    public List<Element> toExpressions(@NotNull DXBC shader) {
        Function function = new Function("log", input.toExpression(shader, output.componentSelection));
        if (isSaturated()) {
