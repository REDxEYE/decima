...
public class Lod extends Opcode {
    ...
    public Lod(int opcodeToken, @NotNull ByteBuffer buffer) {
        super(opcodeToken);
    ...
    public String toString() {
        return "lod %s, %s, %s, %s".formatted(output, inputAddress, inputResource, inputSampler);
    }
    ...
    public List<Element> toExpressions(@NotNull DXBC shader) {
        ...
        return List.of(
            new Assignment(output.toExpression(shader),
                new Expression("%s.Lod(%s, %s)", arguments
                )
            )
        );
    }
