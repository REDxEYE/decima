...
public final class MoveConditional extends Opcode {

    public final OperandToken0 output;
    public final OperandToken0 condition;
    public final OperandToken0 input0;
    public final OperandToken0 input1;


    public MoveConditional(int opcodeToken, @NotNull ByteBuffer buffer) {

        super(opcodeToken);
        output = new OperandToken0(buffer);
        condition = new OperandToken0(buffer);
    ...
    public String toString() {
        return "movc %s, %s, %s, %s".formatted(output, condition, input0, input1);
    }

    @Override
    public List<Element> toExpressions(@NotNull DXBC shader) {
        Element element = new Expression("%s ? %s : %s",
            condition.toExpression(shader, output.componentSelection),
