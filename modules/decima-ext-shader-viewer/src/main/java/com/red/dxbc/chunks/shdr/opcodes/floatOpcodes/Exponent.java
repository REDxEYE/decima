...
public final class Exponent extends Opcode {

    public final OperandToken0 output;
    public final OperandToken0 input;


    public Exponent(int opcodeToken, @NotNull ByteBuffer buffer) {
    ...
    public String toString() {
        return "exp %s, %s".formatted(output, input);
    }

    @Override
    public List<Element> toExpressions(@NotNull DXBC shader) {
        Function function = new Function("exp", input.toExpression(shader, output.componentSelection));
        if (isSaturated()) {
